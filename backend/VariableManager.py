...
class VariableManager:
	...
	def add_var(self, vname, vtype, sprite):
		...
		if sprite is not None:
			...
			self.wv.all_sprites_code[sprite]["vars"][vname] = vtype
		else:
			self.global_vars[vname] = {}
			self.wv.all_sprites_code["vars"][vname] = vtype
	...
	def delete_var(self, vname, sprite):
		if sprite is not None:
			...
			if not self.local_vars[sprite][vname]:
				...
				del self.wv.all_sprites_code[sprite]["vars"][vname]
			else:
			...
			if not self.global_vars[vname]:
				...
				del self.meta_dict[vname]
				del self.wv.all_sprites_code["vars"][vname]
			else:
	...
	def _delete_var_and_ref(self, vname, sprite):
		...
		if sprite is not None:
			for item in self.local_vars[sprite][vname]:
				del self.wv.all_sprites_code[sprite]["code"][item]
	...
	def delete_var_by_name(self, internal_name):
		...
		sprite = self.wv.current_sprite if internal_name.startswith(" v") else None
